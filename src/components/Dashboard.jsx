...
const RECENT_VITALS = MANUAL_READINGS.slice(0, 4).map(r => ({
  date: r.date, bp: `${r.bp_s}/${r.bp_d}`, hr: "--", o2: "--", flag: r.flag,
}));
  ...
  return (
    <div className="stat-card" style={{ cursor: "pointer" }} onClick={() => setOpen(o => !o)}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#dde8f5", letterSpacing: "-1px", lineHeight: 1, marginBottom: 5 }}>{count}</div>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#7eb8d8", marginBottom: 3 }}>Refills This Week</div>
          <div style={{ fontSize: 10, color: "#98afc4", fontFamily: "'DM Mono',monospace" }}>{weekLabel}</div>
        </div>
...
function BPCard() {
  ...
  const bpValue = `${r.bp_s}/${r.bp_d}`;
  const bpDate  = r.date;
  return (
    <div className="stat-card">
      <div style={{ width: 28, height: 3, background: "#4f8ef7", borderRadius: 2, marginBottom: 14, boxShadow: "0 0 10px #4f8ef760" }} />
      <div style={{ fontSize: 26, fontWeight: 700, color: "#dde8f5", letterSpacing: "-1px", lineHeight: 1, marginBottom: 5 }}>{bpValue}</div>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#7eb8d8", marginBottom: 3 }}>Blood Pressure</div>
      <div style={{ fontSize: 10, color: "#98afc4", fontFamily: "'DM Mono',monospace" }}>Recorded {bpDate}</div>
    </div>
  );
}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px" }}>
            ...
            <div>
              ...
              <div style={{ background: "#0b1220", border: "1px solid #111e30", borderRadius: 14, padding: 16 }}>
                ...
                {["Analyze my current health status", "Review my medications for interactions", "Prep questions for nephrology appt"].map((q, i) => (
                  <button key={i} className="ai-btn" style={{ marginBottom: 8, justifyContent: "flex-start", textAlign: "left" }}>
                    <span style={{ color: "#4f8ef7", fontSize: 14 }}>✦</span><span>{q}</span>
                  </button>
                ))}
                <button className="ai-btn" style={{ marginTop: 4, borderStyle: "dashed" }}><span>Custom query...</span></button>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "#0b1220", border: "1px solid #111e30", borderRadius: 14, padding: "18px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "80px 100px 50px 60px", padding: "0 0 8px", borderBottom: "1px solid #0d1a28", marginBottom: 4 }}>
              {["DATE", "BP", "HR", "O2"].map(h => (
